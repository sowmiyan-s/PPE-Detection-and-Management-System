...
and decorator-based wrapping for SQL read queries.
"""
...
# Global cache manager instance for SQL engine
sql_cache = AsyncCacheManager()
mongo_cache = sql_cache  # Backward-compatibility alias
...
def cached(ttl: float = 10.0, tags: list[str] | None = None) -> Callable[[F], F]:
    """Decorator to cache async function results in sql_cache for `ttl` seconds."""
    tag_list = tags or []
    ...
    def decorator(func: F) -> F:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            cached_val = await sql_cache.get(cache_key)
            ...
            if res is not None:
                await sql_cache.set(cache_key, res, ttl=ttl, tags=tag_list)
            return res
