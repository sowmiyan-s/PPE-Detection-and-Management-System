...
export const Route = createFileRoute("/cameras")({
  head: () => ({
    meta: [
      { title: "Camera Management — Cerberus AI Stream Registry" },
      {
        name: "description",
        content:
...
function CamerasPage() {
  ...
  const [formZone, setFormZone] = useState("General Plant Floor");
  ...
  const handleTypeChange = (t: string) => {
    ...
    if (t === "webcam") {
      ...
      setFormUrl("https://www.youtube.com/watch?v=...");
      if (!formName) {
        setFormName("YouTube Live Feed");
      }
    } else {
  ...
  const handleAddCamera = (e: React.FormEvent) => {
    e.preventDefault();
    const camNameSubmitted = formName.trim() || (cameraType === "webcam" ? `Local Webcam (Index ${formUrl})` : "Camera Stream");

    setSubmittingAdd(true);
    const newCam = {
      name: camNameSubmitted,
      source: formUrl.trim() || "0",
      streamUrl: formUrl.trim() || "0",
      type: cameraType,
      zoneId: formZone || "General Plant Floor",
  ...
  const openEditModal = (cam: Camera) => {
    ...
    setEditZone(cam.zoneId || "General Plant Floor");
